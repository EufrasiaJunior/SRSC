...
import static com.mycompany.chat2.Criptografia.generateAESKey;
import java.io.*;
import java.net.*;
import java.util.*;
...
public class MulticastChat extends Thread {
    ...
    protected Criptografia c;

    // Multicast Chat-Messaging
    public MulticastChat(String username, InetAddress group, int port,
            int ttl, MulticastChatEventListener listener) throws IOException, Exception {
    ...
    public void sendMessage(String message) throws IOException, Exception {
        ...
        byte[] msg_cifra = c.encryptMessage(message);
        String cifrada = Base64.getEncoder().encodeToString(msg_cifra);

    
        dataStream.writeLong(CHAT_MAGIC_NUMBER);
        dataStream.writeInt(MESSAGE);
        dataStream.writeUTF(username);
        dataStream.writeUTF(cifrada);
        dataStream.write(msg_cifra);
        dataStream.close();
    ...
    protected void processMessage(DataInputStream istream,
        ...
        byte[] msg_cifra= istream.readAllBytes();
        String decifrada = c.decryptMessage(msg_cifra);

        try {
            listener.chatMessageReceived(username, address, port, decifrada);
        } catch (Throwable e) {
        }
    }
    ...
    public void run() {
        ...
        while (isActive) {
            try {
                ...
                /**
                 * Handler for Interruptions ... WILL DO NOTHING ,,, Used for
                 * debugging / control if wanted ... to notify the loop
                 * interruption
                 */
            } catch (Throwable e) {
